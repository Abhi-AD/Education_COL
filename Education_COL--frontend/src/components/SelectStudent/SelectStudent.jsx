import { Link } from "react-router-dom";

const students = [
     {
          title: "Microsoft (Intern)",
          name: "Karitik Agarwal",
          branch: "CSE",
          degree: "BTech",
          batch: "Col Student",
          stipend: "Rs 1 lakh+/month Stipend",
          imgSrc:
               "https://img.freepik.com/free-photo/young-bearded-man-with-striped-shirt_273609-5677.jpg?uid=R86996227&ga=GA1.1.1555463120.1717780249&semt=ais_user-customized",
          companyImg:
               "https://blogs.microsoft.com/wp-content/uploads/prod/2012/08/8867.Microsoft_5F00_Logo_2D00_for_2D00_screen.jpg",
     },
     {
          title: "Google (Intern)",
          name: "Rojal Sapkota",
          branch: "CSE",
          degree: "BS (USA)",
          batch: "Col Student",
          stipend: "Rs 6 lakh+/month CTC",
          imgSrc:
               "https://img.freepik.com/free-photo/indoor-picture-cheerful-handsome-young-man-having-folded-hands-looking-directly-smiling-sincerely-wearing-casual-clothes_176532-10257.jpg?uid=R86996227&ga=GA1.1.1555463120.1717780249&semt=ais_user-customized",
          companyImg:
               "https://cdn.pixabay.com/photo/2013/01/29/22/07/google-76659_1280.png",
     },
     {
          title: "Samsung (SDE)",
          name: "Arif Nowfel",
          branch: "CSE",
          degree: "BSc (Bangladesh)",
          batch: "Col Student",
          stipend: "Stipend - Confidential",
          imgSrc:
               "https://img.freepik.com/free-photo/happy-bearded-man-business-clothes-looking-camera_171337-11392.jpg?t=st=1719831398~exp=1719834998~hmac=80bf08924fe2abe315221382798d60d1e1ad56a4e3dc323875fa7ea4d3e270e5&w=1060",
          companyImg:
               "https://upload.wikimedia.org/wikipedia/commons/thumb/f/f1/Samsung_logo_blue.png/1200px-Samsung_logo_blue.png",
     },
     {
          title: "Amazon (SDE)",
          name: "Arif Nowfel",
          branch: "CSE",
          degree: "BSc (Bangladesh)",
          batch: "Col Student",
          stipend: "Stipend - Confidential",
          imgSrc:
               "https://img.freepik.com/free-photo/headshot-cute-girl-with-hair-bun-spending-lunch-break-restaurant-with-wooden-walls_273609-9037.jpg?uid=R86996227&ga=GA1.1.1555463120.1717780249&semt=sph",
          companyImg:
               "data:image/png;base64,",
     },
];

const SelectStudent = () => {
     return (
          <div className="container">
               <div className="p-5  mx-auto">
                    <div className="title fw-bold fs-4 text-center text-second">
                         Our Selected Students
                    </div>
                    <div className="row row-cols-1 row-cols-md-2 g-4">
                         {students.map((student, index) => (
                              <div key={index} className="col">
                                   <div className="card border-0 mt-4">
                                        <div className="card mx-auto w-100 shadow bg-second border-0 mt-2 d-flex justify-content-center align-items-center">
                                             <h5 className="card-title fs-4 mt-2 mb-2 bg-light rounded-2 p-3">
                                                  {student.title}
                                             </h5>
                                             <div className="d-flex align-items-center mb-5 d-md-none gap-2">
                                                  <img
                                                       className="img-fluid profile"
                                                       src={student.imgSrc}
                                                       alt="profile"
                                                  />
                                                  <div
                                                       className="card-body bg-light text-left ml-auto"
                                                       style={{
                                                            borderTopLeftRadius: "2rem",
                                                            borderBottomLeftRadius: "2rem",
                                                            width: "20rem",
                                                       }}
                                                  >
                                                       <p>Name : {student.name}</p>
                                                       <p>Branch : {student.branch}</p>
                                                       <p>Degree : {student.degree}</p>
                                                       <p>Batch : {student.batch}</p>
                                                  </div>
                                             </div>
                                             <div className="d-flex align-items-center mb-5 d-none d-md-flex gap-5">
                                                  <img
                                                       className="img-fluid profile"
                                                       src={student.imgSrc}
                                                       alt="profile"
                                                  />
                                                  <div
                                                       className="card-body bg-light text-left ml-auto"
                                                       style={{
                                                            borderTopLeftRadius: "2rem",
                                                            borderBottomLeftRadius: "2rem",
                                                            width: "25rem",
                                                       }}
                                                  >
                                                       <p>Name : {student.name}</p>
                                                       <p>Branch : {student.branch}</p>
                                                       <p>Degree : {student.degree}</p>
                                                       <p>Batch : {student.batch}</p>
                                                  </div>
                                             </div>
                                             <p className="text-dark fw-bold fs-5 w-50">
                                                  {student.stipend}
                                             </p>
                                             <div className="bg-light h-100 w-100 d-flex align-items-center justify-content-center">
                                                  <img
                                                       className="img-fluid img-thumbnail border-0"
                                                       src={student.companyImg}
                                                       alt="company"
                                                       style={{ width: '100%', height: '130px' }}
                                                  />
                                             </div>
                                        </div>
                                   </div>
                              </div>
                         ))}
                    </div>
                    <div className="text-center mb-5 mt-5">
                         <Link
                              type="button"
                              className="text-light btn btn-primary fs-4 bg-second"
                              to={`/sandboxing`}
                         >
                              See More
                              <i className="bi bi-chevron-right" data-aos="fade-left"></i>
                         </Link>
                    </div>
               </div>
          </div>
     );
};

export default SelectStudent;
